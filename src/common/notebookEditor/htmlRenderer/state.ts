import { isCodeBlockJSONNode, isHeadingJSONNode, AttributeType, HeadingAttributes, JSONNode, NodeIdentifier, NodeName } from 'common';
...
export const computeState = (doc: JSONNode): RendererState => {
  ...
  const incorporateNode = (node: JSONNode): boolean/*false when done traversing*/ => {
    if(isCodeBlockJSONNode(node) && node.attrs) {
      ...
      const attrs = node.attrs as HeadingAttributes/*by contract*/;
      updateStack(attrs[AttributeType.Level], codeBlockState);
