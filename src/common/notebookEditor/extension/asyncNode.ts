...
export const AsyncNodeAttributeSpec = {
  ...
  [AttributeType.Status]: noNodeSpecAttributeDefaultValue<AsyncNodeStatus>(),
};
...
export const asyncNodes: Set<NodeName> = new Set([NodeName.DEMO_ASYNCNODE, NodeName.DEMO2_ASYNCNODE]);
...
// (e.g. a demoAsyncNode's attributes can be modified while its promise resolves),
// so the ones that must not exhibit this behavior must be added here
export const nonEditableWhileOperatingAsyncNodes = new Set<NodeName>([NodeName.DEMO2_ASYNCNODE]);
